...
exports.postRoutes.get('/:id', (req, res) => {
    const getPost = post_in_db_repository_1.postRepository.getPostById(req.params.id);
    if (getPost) {
        res.status(200).send(getPost);
    }
    else {
...
exports.postRoutes.put('/:id', base_auth_middlewares_1.baseAuthorizationMiddleware, createPostValidation, (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    const content = req.body.content;
    const blogId = req.body.blogId;
    const updatePost = yield post_in_db_repository_1.postRepository.updatePost(req.params.id, title, shortDescription, content, blogId);
    if (updatePost) {
        res.send(204);
    }
    else {
